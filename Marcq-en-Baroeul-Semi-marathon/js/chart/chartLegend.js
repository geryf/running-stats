class ChartLegend {
    ...
    /**
     * @param array keys    catégories cochées au départ (barres affichées)
     * @param array allKeys toutes les catégories connues de la légende ; par défaut
     *                      les mêmes que `keys`. Les deux diffèrent lors d'un
     *                      redessin, où la légende reste complète alors que seule
     *                      une partie des catégories est cochée.
     */
    constructor(chart, coureurCategorieIdsToCategories, keys, callback, allKeys) {
        this.coureurCategorieIdsToCategories = coureurCategorieIdsToCategories; 
        this.chart = chart;
        this.callbacks = []; 
        // Tant que l'utilisateur n'a pas cliqué dans la légende, l'URL n'est pas
        // touchée : le paramètre n'apparaît qu'à la première sélection manuelle.
        // Voir enableUrlSync(), appelé par les templates une fois le rendu initial terminé.
        this.urlSyncEnabled = false;
        this.addCallback(callback);
        this.initKeyVisibility(keys, allKeys);
    }

    /**
     * Autorise l'écriture de la sélection courante dans l'URL. À appeler après le
     * rendu initial (état par défaut ou état repris de l'URL), pour que seules les
     * sélections faites par l'utilisateur modifient l'adresse de la page.
     */
    enableUrlSync() {
    ...
    /**
     * Lit la sélection de catégories mémorisée dans l'URL (?cat=12,15,agg_F).
     *
     * @param  array orderedKeys catégories connues, dans l'ordre d'affichage de la légende
     * @return array|null        catégories à cocher (éventuellement vide si l'URL demande
     *                           explicitement une sélection vide), ou null si l'URL ne
     *                           contient pas de sélection exploitable — le comportement
     *                           par défaut de la page s'applique alors.
     */
    static readKeysFromUrl(orderedKeys) {
        if (typeof URLSearchParams === 'undefined') {
            return null;
        }
        var raw = new URLSearchParams(window.location.search).get(ChartLegend.URL_PARAM);
        if (raw === null) {
            return null;
        }
        var wanted = {};
        ...
        // On repart de l'ordre d'affichage plutôt que de l'ordre du paramètre : c'est lui
        // qui détermine l'empilement des barres.
        var keys = [];
        ...
        ['agg_M', 'agg_F'].forEach(function(key) {
            if (wanted[key]) {
                keys.push(key);
            }
        });
        // Paramètre présent mais ne désignant aucune catégorie connue (URL d'une autre
        // course, catégorie disparue) : on ignore plutôt que d'afficher un graphique vide.
    ...
    /**
     * Écrit la sélection courante dans l'URL, sans recharger la page ni empiler
     * d'entrée dans l'historique (un clic de légende n'est pas une navigation).
     */
    syncUrl(keys) {
        ...
        url.searchParams.set(ChartLegend.URL_PARAM, keys.join(','));
        window.history.replaceState(null, '', url.toString());
    }
    
    initKeyVisibility(keys, allKeys) {
        this.keysVisibility = [];
        // Toute catégorie connue est enregistrée, même décochée : les bascules
        // « Total catégories hommes/femmes » parcourent ce dictionnaire et ne
        // pourraient pas réactiver une catégorie absente.
...
*/
    ...
    applyVisibility() {
        ...
        d3.selectAll('.M-legend-check').attr('visibility', this.keysVisibility['agg_M'] ? 'true' : 'hidden');
        d3.selectAll('.F-legend-check').attr('visibility', this.keysVisibility['agg_F'] ? 'true' : 'hidden');
    }
}

// Nom du paramètre d'URL portant la sélection de catégories.
ChartLegend.URL_PARAM = 'cat';
